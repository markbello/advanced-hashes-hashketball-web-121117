...
def team_colors(team)
  find_the_team(team).fetch(:colors)
end
...
def player_numbers(team_name)
  find_the_team(team_name)[:players].map{|player| player[:number]}
end

def player_stats(player)
  find_the_player(player).reject {|key, value| key == :player_name}
end

def big_shoe_rebounds
  players.sort_by{|player| player.fetch(:shoe)}.last.fetch(:rebounds)
end
...
def winning_team
  teams.sort_by{|team|
    points = 0
    team.fetch(:players).each{|player|
...
def player_with_longest_name
  players.sort_by{|player| player.fetch(:player_name).length}.last[:player_name]
end
